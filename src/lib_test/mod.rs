...
pub fn test_lsm_tree(bulkwrite_file: &String, workload_file: &String, pool: ThreadPool) {
	...
	{ 
		...
		lib_helper::run(bulkwrite_file, workload_file, pool);
	}
	...
	// let sort_expected = File::create("sorted_expected.txt").expect("error creating file");
	// let mut child = Command::new("sort")
	// 				.args(&["expected.txt"])
	// 				.stdout(Stdio::from(sort_expected))
	// 				.spawn()
	// 				.expect("failed to sort expected");
	
	// let ecode = child.wait()
	// 	.expect("failed to wait on child");
	// assert!(ecode.success());
	
	// let sort_output = File::create("sorted_output.txt").expect("error creating file");
	// let mut child = Command::new("sort")
	// 				.args(&["output.txt"])
	// 				.stdout(Stdio::from(sort_output))
	// 				.spawn()
...
pub fn test_update_heavy_workload(pool: ThreadPool) {
...
pub fn bench_mixed() {
	...
	run_bench(true, true, true, 1_000_000, 100_000, 100_000, &mut workload_file, &mut bulkwrite_file);
}
...
fn bench_read_only() {
	...
	run_bench(true, true, true, 1_000_000, 0, 200_000, &mut workload_file, &mut bulkwrite_file);
}
...
fn bench_read_heavy() {
	...
	run_bench(true, true, true, 1_000_000, 40_000, 160_000, &mut workload_file, &mut bulkwrite_file);
}
...
fn bench_write_only() {
	...
	run_bench(true, true, true, 1_000_000, 200_000, 0, &mut workload_file, &mut bulkwrite_file);
}
...
fn bench_write_heavy() {
	...
	run_bench(true, true, true, 1_000_000, 160_000, 40_000, &mut workload_file, &mut bulkwrite_file);
}
